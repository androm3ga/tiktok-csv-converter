...
with tab_sla:
    st.markdown(
        "TikTok Shop sipariş CSV/Excel'inizi yükleyin → "
        "SLA analizi otomatik hesaplanır → Dönüştürülmüş dosyayı indirin."
    )
    ...
    if uploaded_sla is None:
        ...
        if df_result is not None:
            ...
            if only_breach and "SLA Durumu" in dl_df.columns:
                dl_df = dl_df[dl_df["SLA Durumu"].isin(
                    ["SLA İhlali ❌", "Risk Altında ⚠️", "Geç Kargolama ⚠️"]
                )]
            with dl_col2:
...
> **Not:** `shipping_provider_id` değerlerini TikTok Seller Center → Orders → Bulk Upload Tracking şablonundan doğrulayın.
    ...
    if uploaded_conv is None:
        st.info("👆 Başlamak için depo sisteminizden dışa aktardığınız dosyayı yükleyin.")
    else:
        raw_df     = None
        col_map_c  = {}
        ...
        if raw_df is not None:
            ...
            labels    = {
                ...
                "shipping_provider": "Carrier / Kargo Firması",
            }
            final_map = {}

            ca, cb, cc = st.columns(3)
            for field, widget_col in zip(targets, [ca, cb, cc]):
                auto = col_map_c.get(field, "(boş)")
                if auto not in all_cols:
                    auto = "(boş)"
                chosen = widget_col.selectbox(
                    labels[field], options=all_cols,
                    index=all_cols.index(auto), key=f"col_{field}",
                )
                if chosen != "(boş)":
                    final_map[field] = chosen

            missing = [f for f in ["order_id", "tracking_number"] if f not in final_map]
            if missing:
                st.warning(f"⚠️ Zorunlu alan eşleşmedi: {', '.join(missing)}")
            ...
            if not carrier_col_name:
                ...
                with st.expander("🔍 Otomatik eşleşen carrier'ları gör"):
                    ...
                    for val in raw_df[carrier_col_name].dropna().unique():
                        ...
                        if pid is not None:
                            preview.append({"CSV'deki değer": val, "TikTok Carrier": display, "ID": pid})
                    if preview:
            ...
            if st.button("🔄 TikTok Formatına Dönüştür", type="primary", use_container_width=True):
                ...
                r1.metric("Toplam Satır",        len(raw_df))
                r2.metric("Geçerli ✅",           len(valid_df))
                r3.metric("Hatalı / Atlanan ❌",  len(error_df))

                if len(valid_df) > 0:
                    st.success(f"✅ {len(valid_df)} satır TikTok formatına dönüştürüldü.")
                    st.dataframe(valid_df.head(20), hide_index=True, use_container_width=True)
                    base_c = uploaded_conv.name.rsplit(".", 1)[0]
                    st.download_button(
                        f"📥 {base_c}_tiktok.csv olarak indir",
                        data=to_csv_bytes(valid_df),
                        file_name=f"{base_c}_tiktok.csv",
                        mime="text/csv",
                        use_container_width=True,
                    )
                else:
                ...
                if len(error_df) > 0:
                    base_c = uploaded_conv.name.rsplit(".", 1)[0]
                    with st.expander(f"❌ {len(error_df)} hatalı satırı gör"):
                        st.dataframe(
                        ...
                        st.download_button(
                            ...
                            file_name=f"{base_c}_hatalar.csv",
                            mime="text/csv",
                        )
